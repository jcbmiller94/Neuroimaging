...
# read in text file
x = np.genfromtxt('BCWM_pilot1C_resultsTXT.txt', names = True, dtype = None)
...
# splitting into dictionary with 7 entries for each trial of 24 runs 
...
for i in range(len(dictionary)):
	...
	a = arr[0,0] # first raw start time of run 
	b = arr[0,2] # first raw probe time of run 
	for k in range(arr.shape[0]):
		arr[k,0] -= a # setting first time of each run to zero, all other times in this run relative to this point
		arr[k,1] = arr[k,0] + 4.0 # start of delay, adding 4 s
		arr[k,2] -= a # setting first probe time relative to start time of run 
...
# text file of onset times sorted in order of run 
...
# text file of onset times sorted by trial type
# np.savetxt('Onsets_Concatenated_session2.txt', df)
...
np.savetxt('/home/despoB/jam124/BiCoWM/derivatives_test/s01/GLM_multivariate/Multivariate_TrialType_Order_session2.txt', TT_vector) 
...
for i in range(len(multi_onsets)):
	...
	io.savemat('/home/despoB/jam124/BiCoWM/batch_scripts/Conditions_session2/Conditions' + str(i+1) + '.mat', multi_conditions[i])
